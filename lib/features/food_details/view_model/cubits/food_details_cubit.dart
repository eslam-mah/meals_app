...
class FoodDetailsCubit extends Cubit<FoodDetailsState> {
  ...
  // Private constructor
  FoodDetailsCubit._internal(this._foodRepository)
  ...
  Future<void> loadFoodDetails(String foodId) async {
    try {
      ...
      emit(
        initialState.copyWith(totalPrice: initialState.calculateTotalPrice()),
      );
    } catch (e) {
  ...
  void selectSize(FoodSize? size) {
    ...
    emit(updatedState.copyWith(totalPrice: updatedState.calculateTotalPrice()));
  }
  ...
  void selectBeverage(FoodBeverage beverage) {
    final currentBeverages = List<FoodBeverage>.from(
      state.selectedBeverage ,
    );

    if (currentBeverages.contains(beverage)) {
      currentBeverages.remove(beverage);
    } else {
      currentBeverages.add(beverage);
    }
    final updatedState = state.copyWith(selectedBeverage: currentBeverages);
    emit(updatedState.copyWith(totalPrice: updatedState.calculateTotalPrice()));
  }

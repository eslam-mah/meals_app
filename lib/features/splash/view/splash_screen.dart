...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _checkConnectivityAndProceed() async {
    ...
    _log.info(
      'Initial connectivity check: ${_isConnected ? "Connected" : "Disconnected"}',
    );
    ...
    ConnectivityDialog.show(
      ...
      onConnected: () {
        if (mounted && !_isNavigating) {
          _log.info(
            'Connection restored from dialog, proceeding with navigation',
          );
  ...
  Future<void> _navigateToNextScreen() async {
    ...
    try {
      ...
      _log.info(
        'Direct Supabase authentication check: $isDirectlyAuthenticated',
      );
      ...
      if (isDirectlyAuthenticated) {
        ...
        _log.info(
          'User is not authenticated - navigating to main view (limited access)',
        );

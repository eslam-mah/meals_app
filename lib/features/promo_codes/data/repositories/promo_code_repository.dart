...
class PromoCodeRepository {
  ...
  Future<PromoCodeModel?> validatePromoCode(String code, String userId) async {
    try {
      ...
      _log.info(
        'Parsed promo code: ID=${promoCode.id}, Code=${promoCode.code}, Type=${promoCode.type}, Percentage=${promoCode.percentage}',
      );
      ...
      final hasUserUsed = await _usageRepository.hasUserUsedPromoCode(
        userId,
        promoCode.id,
      );
      if (hasUserUsed) {
        _log.warning(
          'User $userId has already used promo code ${promoCode.code} (ID: ${promoCode.id})',
        );
        return null;
      }

      _log.info(
        'Promo code validated successfully: $code (ID: ${promoCode.id})',
      );
      return promoCode;
    } catch (e) {
